...
public class Solution {
    ...
    public Item getProxy(Class<? extends Item> tClass, Class... interfaces) {
        Class[] intrs = new Class[interfaces.length + 1];
        intrs[0] = tClass;
        for (int i = 0; i < interfaces.length; i++) {
            intrs[i] = interfaces[i];
        }
        intrs[intrs.length - 1] = tClass;
        return (Item) Proxy.newProxyInstance(tClass.getClassLoader(), intrs, new ItemInvocationHandler());
    }
}
