...
//            is.read(arr);
//            is.close();
//            entries.put(zipEntry, arr);
//        }
//        ZipOutputStream zos = new ZipOutputStream(new FileOutputStream(args[1]));
//        for (Map.Entry<ZipEntry, byte[]> e : entries.entrySet()) {
//            if (e.getKey().getName().contains("/")) {
//                if (!e.getKey().getName().substring(e.getKey().getName().lastIndexOf("/") + 1).equals(fileName.getName())) {
//                    zos.putNextEntry(new ZipEntry(e.getKey().toString()));
//                    zos.write(e.getValue());
//                } else {
//                    zos.putNextEntry(new ZipEntry(args[1].substring(args[1].lastIndexOf("/") + 1, args[1].lastIndexOf(".zip")) + "/new/" + fileName.getName()));
//                    Files.copy(fileName.toPath(), zos);
//                }
//            } else {
//                if (!e.getKey().getName().equals(fileName.getName())) {
//                    zos.putNextEntry(new ZipEntry(e.getKey().toString()));
//                    zos.write(e.getValue());
//                } else {
//                    zos.putNextEntry(new ZipEntry(args[1].substring(args[1].lastIndexOf("/") + 1, args[1].lastIndexOf(".zip")) + "/new/" + fileName.getName()));
