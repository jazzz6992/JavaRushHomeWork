...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory {
        ...
        int thisFactoryNumber = factoryNumber.get();
        AtomicInteger threadNumber = new AtomicInteger(0);

        public AmigoThreadFactory() {
            thisFactoryNumber = factoryNumber.incrementAndGet();
        }

        @Override
        public Thread newThread(Runnable r) {
            threadNumber.incrementAndGet();
            Thread t = new Thread(r, Thread.currentThread().getThreadGroup().getName().toString() + "-pool-" + thisFactoryNumber + "-thread-" + threadNumber);
            return t;
        }
    }
